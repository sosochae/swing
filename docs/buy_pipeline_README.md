...
16. [펀더멘털 스크리닝 (ScreenerMCP / KavoutMCP)](#16-펀더멘털-스크리닝-screenermcp--kavoutmcp)
17. [자동 트리거 (EventWatcher)](#17-자동-트리거-eventwatcher)
18. [CLAUDE CLI 모드](#18-claude-cli-모드)
...
orchestrator/steps/buy_steps.py  ← BuySteps (각 Step 구현)
        │
        ├─ core/analysis.py       ← 기술 분석 / 필터 / 시나리오 / RRE
        ├─ core/api_fetcher.py    ← yfinance / Finnhub / 옵션 체인
        ├─ core/llm.py            ← OpenRouter / CLAUDE CLI LLM / 뉴스 검색
        │                            rank_and_pick / fetch_url_as_markdown
        ├─ core/obsidian.py       ← Obsidian REST API
        ├─ core/slack.py          ← Slack Bot
        ├─ core/parsers.py        ← Finviz / Summary / Earnings / Positions 파서
        └─ core/state.py          ← 스냅샷 / 감사 로그 / Requeue

추가 MCP 서버 (독립 실행):
  servers/screener_mcp/server.py  ← ScreenerMCP (Finviz 기반 펀더멘털 스크리닝)
  servers/kavout_mcp/server.py    ← KavoutMCP  (Kavout CSV 기반 펀더멘털 스크리닝)

자동 트리거:
  orchestrator/event_watcher.py   ← 파일 변경 감지 → 파이프라인 자동 실행
```
...
```dotenv
# LLM 프로바이더 선택 ("openrouter" 또는 "claude_cli")
LLM_PROVIDER=openrouter

# OpenRouter (LLM_PROVIDER=openrouter 일 때)
...
LLM_MODEL_RANK_AND_PICK=deepseek/deepseek-chat       # 뉴스 기사 선별 모델
LLM_MODEL_CATALYST_WEB=deepseek/deepseek-v4-flash    # 촉매 웹검색 분석 모델
LLM_MODEL_GAP_ANALYSIS=deepseek/deepseek-chat        # 갭 분석 모델

# CLAUDE CLI 모드 (LLM_PROVIDER=claude_cli 일 때)
CLAUDE_CLI_MODEL=claude-sonnet-4-6  # claude -p 호출 시 사용 모델

# Research Agent
RESEARCH_MODEL=google/gemini-2.5-flash  # 독립 리서치 에이전트 모델
RESEARCH_FETCH_TOP_N=5    # 상위 N개 URL 본문 수집
```
...
| Finviz 전체 목록 | `FINVIZ_FILE` (기본: `Y:\내 드라이브\어닝\finviz_all_rows.txt`) | Finviz 수동 내보내기 |
...
> **참고**: `FINVIZ_FILE`이 없으면 Step 0에서 E200 경고를 기록하지만 파이프라인은 계속 실행된다. Finviz 데이터가 없으면 `StockDetail`의 일부 필드(주봉 RSI/ADX, 52주 위치, 멀티 기간 수익률 등)를 yfinance/summary 폴백으로 대체한다.
...
# [CLI] 디렉토리 생성 + 환경 검증 + Obsidian ping
python scripts/health_check.py --setup
```

종료 코드: 0 = 통과, 1 = 환경변수 누락 또는 Obsidian 연결 실패
...
```
[사전 준비] — 매번 실행 전 아래 순서 필수
  ① fetch_kavout.py → Kavout CSV 생성
        Y:\내 드라이브\Data\kavout_YYYYMMDD.csv

  ② run_kavout_screener.py → 랭킹 산출 + Top-3 내보내기
        Y:\내 드라이브\Data\kavout_top.json  (AppScript 병합용)

  ③ AppScript → Summary JSON 생성 (②의 top3를 "종목" 탭 티커와 중복 제거 후 병합)
        R:\내 드라이브\마켓 수치\summary_YYYYMMDD.json

  ④ K어닝 분석.md 최신화 (어닝콜 텍스트 + frontmatter)

  ⚠️ 순서 필수: AppScript가 kavout_top.json을 읽어 상위 3종목을 병합하므로
     ①②가 ③보다 반드시 먼저 실행되어야 함.
     kavout_top.json의 date가 오늘이 아니면 AppScript는 병합 스킵 (stale 방지).
        │
        ▼
[파이프라인 실행] [CLI] or [MCP]
...
### 4.1 사전 준비 — 매번 실행 전 순서 필수

```bash
# ① Kavout CSV 수집
.venv\Scripts\python scripts\fetch_kavout.py
.venv\Scripts\python scripts\fetch_kavout.py --universe sp500   # SP500 전체

# ② Kavout 스크리너 → kavout_top.json 생성 (AppScript 병합용)
...
# ③ AppScript 실행 (kavout_top.json → Summary JSON 병합)
#    → Google Sheets AppScript 수동 실행
...
```bash
# [CLI] 전체 watchlist (캐시 무시 — 기본값)
.venv\Scripts\python scripts\run_buy_pipeline.py

# [CLI] LLM 캐시 재사용 (빠른 재실행)
.venv\Scripts\python scripts\run_buy_pipeline.py --use-cache

# [CLI] 특정 종목만
.venv\Scripts\python scripts\run_ticker.py TSLA
.venv\Scripts\python scripts\run_ticker.py AAPL MSFT NVDA
.venv\Scripts\python scripts\run_ticker.py TSLA --use-cache
```

### 4.3 매도 파이프라인 실행

```bash
# [CLI] 전체 포지션 분석 (LLM 포함)
.venv\Scripts\python scripts\run_sell_pipeline.py

# [CLI] 경량 rule-based 매도 체크 (LLM 없음, 빠름)
...
```bash
# [CLI] 30분 간격 급락 감지 (별도 터미널에서 상시 실행)
.venv\Scripts\python scripts\position_monitor.py

# [CLI] 간격 지정
.venv\Scripts\python scripts\position_monitor.py --interval 15

# [CLI] 1회만 실행
.venv\Scripts\python scripts\position_monitor.py --once
```

급락 감지 기준: `entry_stock_price` 대비 `RESEARCH_DROP_THRESHOLD`(-3.0%) 이하 → `run_drop_research()` → Obsidian + Slack 알림.
...
모든 CLI는 `cd C:\MCP\Swing` 후 `.venv\Scripts\python` prefix로 실행.

**매수 파이프라인**

| 스크립트 | 옵션 | 설명 |
|---------|------|------|
| `scripts/run_buy_pipeline.py` | `--use-cache` / `--no-cache` | 전체 watchlist 매수 분석 |
| `scripts/run_ticker.py TSLA [...]` | `--use-cache` | 특정 종목 매수 분석 |

**매도 파이프라인**

| 스크립트 | 옵션 | 설명 |
|---------|------|------|
| `scripts/run_sell_pipeline.py` | 없음 | 전체 포지션 LLM 매도 분석 |
| `scripts/run_sell_check.py` | `--ticker LRCX`, `--verbose` | 경량 rule-based 매도 체크 (LLM 없음) |

**데이터 수집 / 스크리닝**

| 스크립트 | 옵션 | 설명 |
|---------|------|------|
| `scripts/fetch_kavout.py` | `--universe sp500` | Kavout CSV 스크래핑 |
| `scripts/run_kavout_screener.py` | `--refresh-earnings`, `--top N` | Kavout 펀더멘털 스크리닝 + kavout_top.json 생성 |
| `scripts/run_screener.py` | `--force-refresh`, `--top N` | Finviz 기반 펀더멘털 스크리닝 |

**Requeue / 유지보수**

| 스크립트 | 옵션 | 설명 |
|---------|------|------|
| `scripts/run_requeue.py` | `--add NVDA --reason "..."`, `--list` | Requeue 파이프라인 실행 / 관리 |
| `scripts/position_monitor.py` | `--interval N`, `--once` | 포지션 급락 감지 데몬 |
| `scripts/health_check.py` | `--setup` | 환경 검증 + 디렉토리 초기화 |
| `scripts/validate_rre_thresholds.py` | 없음 | RRE 임계값 과거 케이스 검증 |

**`_` 접두사 스크립트** — 디버깅·개발용 유닛 테스트, 운영에서는 사용하지 않음:
`_parse_check.py`, `_summary_check.py`, `_note_check.py`, `_oi_unit_test.py`, `_obsidian_test.py`, `_quick_check.py`, `_compare_notes.py`, `_verify_improvements.py`, `_rvol_check.py`, `_full_report_test.py`, `_render_test.py`, `_kavout_stock_pages.py`

### 5.2 SwingMCP Tools 전체 목록 (12개)
...
| `read_note` | Obsidian 노트 읽기 | `path` |
| `get_ticker_info` | 종목 매수 보고서 + 정량 지표 + 포지션 조회 | `ticker` |

**`read_note` 사용 예시** (독립 리서치에 활용):
```json
{ "path": "swing-procedure/notes/buy/2026-07-01.md" }
{ "path": "Research/NVDA_2026-07-01.md" }
```

**`get_ticker_info` 사용 예시**:
```json
{ "ticker": "NVDA" }
```
→ 반환: Research 노트 전문 (`quant_data`) + 포지션 현황 (`position`)

### 5.3 ScreenerMCP Tools

| Tool | 설명 |
|------|------|
| `run_fundamental_screen` | Finviz 기반 3단계 펀더멘털 스크리닝 실행 |
| `screener_health_check` | 데이터 파일·Obsidian·Slack 연결 확인 |

### 5.4 KavoutMCP Tools

| Tool | 설명 |
|------|------|
| `run_kavout_screen` | Kavout CSV 기반 3단계 펀더멘털 스크리닝 실행 |
| `kavout_health_check` | 데이터 파일·연결 확인 |

### 5.5 MCP 서버 실행

```bash
# [CLI] SwingMCP 서버 (Buy/Sell/Requeue + 독립 리서치 Tools)
python servers/swing_mcp/server.py

# [CLI] ScreenerMCP 서버 (Finviz 기반 펀더멘털 스크리닝)
python servers/screener_mcp/server.py

# [CLI] KavoutMCP 서버 (Kavout 기반 펀더멘털 스크리닝)
python servers/kavout_mcp/server.py
```

모든 서버는 stdio JSON-RPC 프로토콜로 동작하며, Claude Desktop / Roo Code에서 자동 연결됩니다.
...
| F8_PARABOLIC_TOP | 주봉 RSI > 임계값 AND 주봉 ADX > 임계값 (blow-off top) | 아니오 — DA 차감만 적용 |

F4(상장폐지 위험)만 Hard Stop. F8은 **기록 전용** (종목은 분석 계속, Step 6에서 DA 차감). 나머지는 기록만 하고 분석 계속.

**F8 판정 함수**: `core/analysis.py` → `is_parabolic_top(weekly_rsi, weekly_adx)`
- Step 4에서 판정 → `ctx.filter_failures[ticker]`에 `"F8_PARABOLIC_TOP"` 추가
- Step 6에서 해당 플래그 확인 → `DA_BUY_PARABOLIC_PENALTY` 차감
...
③ DDG 결과 품질 선별 + 본문 fetch (research_agent 패턴)  ← 신규
   - rank_and_pick()으로 LLM이 제목 보고 상위 3개 선별
   - fetch_url_as_markdown()으로 실제 기사 본문 3,000자 수집
   - 선별 기사를 뉴스 목록 앞에 배치
...
**rank_and_pick**: `core/llm.py` → LLM이 뉴스 제목 목록을 보고 관련성 높은 상위 N개 선택
**fetch_url_as_markdown**: `core/llm.py` → URL 본문을 마크다운으로 fetch (최대 max_chars)
...
| 내부자 순매도 (3단계 티어) | -5~-20점 | 순매도 금액 구간별 차등 |
| 최근 EPS 미스 | -5점 | eps_surprise_pct < -5% |
| 주봉 RSI 과매수 (고위험) | -5~-15점 | 주봉 RSI ≥ 임계값 (롱콜만 적용) |

**RRE (Reversal Risk Engine) — 고점 회피 DA** (롱콜 방향만 적용):

| 패턴 | 차감 | 조건 요약 |
|------|------|---------|
| F8 포물선 경고 | 별도 상수 | `F8_PARABOLIC_TOP` 플래그 기록 시 |
| Wyckoff 분포 패턴 (3/4) | 별도 상수 | 1개월 급등 + 내부자 매도 + RVOL 급감 + MACD 이탈 중 3개 충족 |
| Wyckoff 분포 패턴 (4/4) | 확정 패널티 | 4개 조건 모두 충족 |
| UTAD (가짜 돌파) | 별도 상수 | 급등 상태에서 단일 세션 급등 OR 업그레이드 직후 |
| Max Pain 이격 (경고) | 별도 상수 | 현재가와 Max Pain 이격 > 경고 임계값 |
| Max Pain 이격 (위험) | 위험 패널티 | 이격 > 위험 임계값 |
| P/C 비율 군중 과열 | 별도 상수 | P/C ratio < 과열 임계값 |
| 촉매 공백 + 급등 | 별도 상수 | 다음 실적 > N일 후인데 1개월 수익률 > 임계값 |
| GAAP 하락 + DCF 버블 | 별도 상수 | GAAP 성장률 < 0 또는 DCF CAGR 극단 과대평가 |
| 섹터 대비 초과 | 별도 상수 | 종목 vs 섹터 초과 상승 > 임계값 |

**RRE 관련 함수** (`core/analysis.py`):
- `is_parabolic_top(weekly_rsi, weekly_adx)` → bool
- `is_max_pain_plausible(price, max_pain)` → bool
- `calculate_reversal_risk_score(...)` → float (0~100, 높을수록 위험)

모든 종목은 DA 차감 후에도 다음 Step으로 진행. 탈락 없음.
...
| RRE 임계값 조정 | `shared/strategy.py` | `RRE_F8_WEEKLY_RSI_MIN`, `RRE_F8_WEEKLY_ADX_MIN`, `RRE_DIST_*`, `RRE_UTAD_*` 등 |
| LLM 모델 교체 | `.env` | `LLM_MODEL_BUY_RESEARCH`, `LLM_MODEL_BUY_TECH_NARRATIVE`, `LLM_MODEL_RANK_AND_PICK` |
| LLM 프로바이더 전환 | `.env` | `LLM_PROVIDER=claude_cli` (월정액 사용) 또는 `openrouter` |
| 뉴스 RSS 추가 | `shared/rss_feeds.json` | `market` 또는 `tickers` 섹션 |
| Obsidian 노트 경로 변경 | `shared/config.py` | `BUY_NOTE_PATH_TEMPLATE` |
| 파일 감시 debounce 조정 | `shared/config.py` | `WATCHER_DEBOUNCE_SECONDS` (기본 30초) |
...
[Finviz finviz_all_rows.txt]
  FinvizRow.weekly_rsi / weekly_adx  → Step 4  → F8_PARABOLIC_TOP 판정    → ctx.filter_failures
  FinvizRow.weekly_rsi / weekly_adx  → Step 6  → RRE 고점 회피 DA 차감
  StockDetail.rel_volume             → Step 4  → technical bridge           → RVOL 점수
  StockDetail.return_1m              → Step 6  → RRE Wyckoff / UTAD 패턴   → DA 차감
...
[Kavout CSV]
  ...
  KavoutRow.return_1m               → Step 6  → RRE Wyckoff 분포 패턴     → DA 차감
...
│
├── scripts/
│   ├── run_buy_pipeline.py        ← [CLI] 전체 watchlist 매수 분석
│   ├── run_ticker.py              ← [CLI] 특정 종목 매수 분석
│   ├── run_sell_pipeline.py       ← [CLI] 전체 포지션 LLM 매도 분석
│   ├── run_sell_check.py          ← [CLI] 경량 rule-based 매도 체크
│   ├── run_kavout_screener.py     ← [CLI] Kavout 스크리닝 + kavout_top.json 생성
│   ├── run_screener.py            ← [CLI] Finviz 기반 펀더멘털 스크리닝
│   ├── run_requeue.py             ← [CLI] Requeue 파이프라인 + 관리
│   ├── fetch_kavout.py            ← [CLI] Kavout CSV 스크래핑
│   ├── position_monitor.py        ← [CLI] 포지션 급락 감지 데몬
│   ├── health_check.py            ← [CLI] 환경 검증 + 디렉토리 초기화
│   └── validate_rre_thresholds.py ← [CLI] RRE 임계값 과거 케이스 검증
│
├── orchestrator/
│   ├── pipelines.py               ← BuyPipeline / SellPipeline / RequeuePipeline
│   ├── engine.py                  ← PipelineEngine (MCP 싱글톤)
│   ├── event_watcher.py           ← 파일 변경 감지 → 파이프라인 자동 트리거
│   └── steps/
│       ├── buy_steps.py           ← BuySteps Step 0~13
│       └── sell_steps.py          ← SellSteps Step 0~13
│
├── servers/
│   ├── swing_mcp/
│   │   └── server.py              ← SwingMCP stdio 서버 (12개 Tool 노출)
│   ├── screener_mcp/
│   │   └── server.py              ← ScreenerMCP (Finviz 기반 펀더멘털 스크리닝)
│   └── kavout_mcp/
│       └── server.py              ← KavoutMCP (Kavout 기반 펀더멘털 스크리닝)
│
├── core/
...
│   │                                  is_parabolic_top / is_max_pain_plausible
│   │                                  calculate_reversal_risk_score  (RRE)
...
│   ├── fundamental_screener.py    ← rank_universe (Momentum/Fundamental/Catalyst/Kavout 점수화)
│   ├── obsidian.py                ← ObsidianClient (save_buy_note / read_note / ping / write_watchlist)
│   ├── slack.py                   ← SlackClient (send_buy_result / send_fatal_error)
│   ├── parsers.py                 ← parse_finviz / parse_finviz_detail / load_latest_summary
│   │                                  parse_kavout_universe / parse_earnings / parse_positions
...
│   ├── strategy.py                ← 전략 상수 단일 소스 (RRE 임계값 포함)
...
│   │                                  FinvizRow / StockDetail / ScreenerResult
│   │                                  FundamentalScoreResult / EarningsCallAnalysis
...
│
├── docs/
│   ├── reversal_risk_engine_design.md  ← RRE 설계 문서
│   ├── kavout_screener_README.md       ← Kavout 스크리너 사용 설명서
│   └── buy_pipeline_README.md         ← 이 문서 (docs 복사본)
│
...
## 16. 펀더멘털 스크리닝 (ScreenerMCP / KavoutMCP)

Buy Pipeline과 독립적으로 실행하는 별도 스크리닝 파이프라인. 3단계 구조:

```
[단계 1] 유니버스 파싱
  - ScreenerMCP: Finviz finviz_all_rows.txt → list[FinvizRow] → Finviz 상세 페이지 크롤
  - KavoutMCP:   kavout_*.csv → list[KavoutRow]

[단계 2] LLM 어닝콜 분석 (core/earnings_analyzer.py)
  - K어닝 분석.md / K어닝 분석_today.md 에서 종목별 섹션 추출
  - K어닝콜_output/ 의 어닝콜 텍스트 파일 → LLM 분석
  - 출력: EarningsCallAnalysis (guidance_direction, mgmt_tone, eps_surprise_pct 등)

[단계 3] 점수화 + 랭킹 (core/fundamental_screener.py → rank_universe())
  - Momentum Score  (0~100): RSI 20% + Rel Volume 20% + 52주위치 20% + SMA추세 20%
                              + 멀티기간수익률 20% (Kavout 있을 때)
  - Fundamental Score (0~100): 매출성장YoY 40% + EPS서프라이즈 25% + 영업이익률 35%
  - Catalyst Score  (0~100): 가이던스 60% + 경영진 톤 40%
  - Kavout AI Score (0~100): stock_rank_score 직접 사용
...
**출력**:
- Obsidian: `swing-procedure/screener/kavout/{date}.md` (Top 10 표 + 전체 목록)
- Slack: Top 5 요약

**실행**:
```json
// [MCP] ScreenerMCP
{ "tool": "run_fundamental_screen" }

// [MCP] KavoutMCP
{ "tool": "run_kavout_screen" }
```

---

## 17. 자동 트리거 (EventWatcher)

`orchestrator/event_watcher.py` — `watchfiles` 라이브러리 기반 비동기 파일 감시.

**감시 이벤트 → 트리거 매핑**:

| 이벤트 | 조건 | 트리거 동작 |
|--------|------|------------|
| `summary_YYYYMMDD_HHMMSS.json` 신규 생성 | `SUMMARY_DIR`에서 파일명 패턴 일치 | Buy Pipeline 자동 실행 |
| `positions.md` 변경/생성 | 파일명 일치 | Sell Pipeline 자동 실행 |
| `K어닝콜_output/` 신규 파일 | 경로에 `K어닝콜_output` 포함 | 어닝 분석 자동 실행 |
| `watchlist.md` 변경 | 파일명 일치 | 로깅만 (다음 Buy에서 반영) |
| `requeue.json` 변경 | `status=ready` 항목 존재 시 | Requeue Pipeline 실행 |

**설정**:
- Debounce: 30초 (`WATCHER_DEBOUNCE_SECONDS`)
- 감시 경로가 없으면 로컬 현재 디렉토리 폴백
- 모든 이벤트는 `asyncio.Task`로 비동기 실행 (동시 다중 트리거 가능)

**EventWatcher는 MCP 서버나 파이프라인에 자동 내장되지 않는다.** 별도로 실행해야 하며, 현재는 수동으로 `PipelineEngine`과 연결하여 사용한다.

---

## 18. CLAUDE CLI 모드

`LLM_PROVIDER=claude_cli`로 설정하면 OpenRouter API 대신 `claude -p` 서브프로세스로 LLM을 호출한다.

**사용 목적**: Claude Pro/Max 월정액 계정을 사용하여 OpenRouter API 비용 없이 Claude 모델 호출.

**두 가지 LLM 프로바이더 비교**:

| 항목 | `openrouter` (기본) | `claude_cli` |
|------|---------------------|-------------|
| 인증 | `OPENROUTER_API_KEY` 필요 | 로그인된 claude 세션 사용 |
| 과금 | OpenRouter 종량제 | Claude 월정액 한도 내 |
| JSON 강제 | 지원 (response_format) | 미지원 → OpenRouter fallback |
| 병렬 호출 | 빠름 | 서브프로세스 오버헤드 있음 |
| 모델 범위 | OpenRouter 전체 모델 | Claude 모델만 |

**동작 방식**:
```
LLM 호출 요청
    │
    ├─ response_format=json_object 인 경우
    │       → 자동으로 OpenRouter로 라우팅 (Claude CLI는 JSON 강제 불가)
    │
    └─ 일반 텍스트 응답인 경우
            → claude -p "프롬프트" --model {CLAUDE_CLI_MODEL} 서브프로세스 실행
```

**설정**:
```dotenv
LLM_PROVIDER=claude_cli
CLAUDE_CLI_MODEL=claude-sonnet-4-6   # claude -p 에 넘길 모델 ID
# OpenRouter API 키 없어도 동작 (단, JSON 요청 시엔 필요)
```

**주의사항**:
- `claude` CLI가 PATH에 설치되어 있어야 함
- JSON 응답이 필요한 호출 (Step 2 레짐, Step 5 감성 분석 JSON 파싱 등)은 OpenRouter로 자동 fallback → `OPENROUTER_API_KEY`가 필요할 수 있음
- 서브프로세스 오버헤드로 인해 병렬 LLM 호출 시 latency 증가 가능
- `LLM_PROVIDER`를 변경하면 전체 파이프라인에 즉시 적용됨 (모델별 오버라이드는 `.env`의 `LLM_MODEL_*` 변수로 개별 지정 가능)

---

*작성 기준: 2026-07-01 / 코드 기준 `scripts/run_buy_pipeline.py` · `orchestrator/steps/buy_steps.py` · `orchestrator/event_watcher.py` · `servers/swing_mcp/server.py` · `servers/screener_mcp/server.py` · `servers/kavout_mcp/server.py` · `core/fundamental_screener.py` · `shared/config.py` · `shared/strategy.py`*
