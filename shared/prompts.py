...
- DI+: {{ di_plus }}  |  DI-: {{ di_minus }}  (⚠️ DI->DI+ 이면 하락추세 우세)
...
- **레짐 및 DI 방향 최우선 원칙**: regime_status가 unfavorable이거나 DI->DI+이면, 방향({{ direction }})과 관계없이 리스크와 부정적 근거를 먼저, 강세 논거는 가능성으로만 서술할 것
...
- 플래그: {{ flags }}
- DTE 긴급도: {{ dte_urgency }}
- 잔여 계약 수: {{ remaining_contracts }}계약 (1계약이면 PARTIAL_EXIT 불가)
- 추세: {{ trend_status }}
- 자금 유입: {{ capital_flow }}
...
- 수익/손실 여부: {{ pnl_status }}
- 상세: {{ pnl_detail }}
- ※ "이익을 lock-in"이나 "수익 확정" 표현은 수익 중일 때만 사용할 것
...
7. **1계약 포지션(contracts=1)에서는 PARTIAL_EXIT 불가 → HOLD 또는 FULL_EXIT만 선택**
...
- 유형: {{ option_type }}
- 진입 프리미엄: ${{ entry_premium }}
- 현재 프리미엄: ${{ current_premium }}
- 미실현 손익: ${{ unrealized_pnl }}
- 실현 손익: ${{ realized_pnl }} (DRY-RUN이면 0, 미실현 손익 기준으로 판단)
- 보유 기간: {{ days_held }}일
- ※ outcome은 미실현 손익 기준으로 판단 (실현 손익이 0이면 미실현 사용)
