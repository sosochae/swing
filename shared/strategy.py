...
# 다중 신호 방향 스코어 임계값
DIRECTION_SCORE_CALL_MIN: int   = 3     # score ≥ +3 → long_call 확정
DIRECTION_SCORE_PUT_MAX:  int   = -3    # score ≤ -3 → long_put 후보
DIRECTION_PUT_ADX_MIN:    float = 18.0  # long_put 허용 최소 ADX (추세 존재)
DIRECTION_PUT_VIX_MAX:    float = 30.0  # long_put 허용 최대 VIX (옵션 비용)

# 개별 종목 주봉 방향 오버라이드 임계값
# Weekly DI+ >> DI- → 매크로와 무관하게 개별 종목은 long_call
# Weekly DI- >> DI+ → 매크로와 무관하게 개별 종목은 long_put
STOCK_DIR_WEEKLY_DI_RATIO: float = 1.5   # DI 비율 기준 (ex: DI+ > DI- × 1.5)
STOCK_DIR_WEEKLY_ADX_MIN:  float = 30.0  # 추세 강도 최소값 (ADX 30 이상 = 유효한 추세)
...
KAVOUT_SR_LOW_SCORE:          float = 30.0
KAVOUT_SR_HIGH_SIGNAL_BONUS:  int   = 1
KAVOUT_SR_HIGH_SCORE_BONUS:   float = 5.0
KAVOUT_SR_LOW_SIGNAL_PENALTY: int   = -1
KAVOUT_SR_LOW_SCORE_PENALTY:  float = -8.0

# Quality Score (0~100 — 파이프라인에 없던 품질 신호)
KAVOUT_QUALITY_THRESHOLD:     float = 80.0
KAVOUT_QUALITY_SIGNAL_BONUS:  int   = 1
KAVOUT_QUALITY_SCORE_BONUS:   float = 3.0

# ROIC (자본효율 — 완전 새 신호, %)
KAVOUT_ROIC_THRESHOLD:        float = 15.0
KAVOUT_ROIC_SIGNAL_BONUS:     int   = 1
KAVOUT_ROIC_SCORE_BONUS:      float = 3.0

# 12개월 수익률 (장기 추세 지속성 — 52W 위치와 다른 관점, %)
KAVOUT_RETURN_12M_THRESHOLD:  float = 50.0
KAVOUT_RETURN_SIGNAL_BONUS:   int   = 1
KAVOUT_RETURN_SCORE_BONUS:    float = 3.0

# 애널리스트 추천 보정 (yfinance recom: 1=Strong Buy ~ 5=Sell)
ANALYST_BUY_THRESHOLD:       float = 2.0
ANALYST_SELL_THRESHOLD:      float = 4.0
ANALYST_SELL_SCORE_PENALTY:  float = -5.0

SHORT_FLOAT_SQUEEZE_THRESHOLD: float = 15.0
...
DA_BUY_INSIDER_API_PENALTY:    float = -10.0         # API 내부자 매도 차감
DA_BUY_EPS_API_PENALTY:        float = -5.0          # API EPS 서프라이즈 차감
...
DA_BUY_INSIDER_API_PCT:        float = -10.0         # API insider_trans_pct 기준
...
SELL_INSIDER_SELL_PCT:       float = DA_BUY_INSIDER_API_PCT  # -10.0
...
FSCORE_WEIGHT_MOMENTUM:    float = 0.50   # RSI + RVOL + 52W + SMA + 멀티수익률
FSCORE_WEIGHT_FUNDAMENTAL: float = 0.0    # 순위 계산에서 제외 (점수는 노트용으로 유지)
FSCORE_WEIGHT_CATALYST:    float = 0.35   # 가이던스 + 경영진 톤 (있는 경우)
FSCORE_WEIGHT_KAVOUT:      float = 0.15   # Kavout AI Stock Rank 점수

# Catalyst 없는 종목용 재분배 비율
FSCORE_NO_CATALYST_MOMENTUM:    float = 0.85
FSCORE_NO_CATALYST_FUNDAMENTAL: float = 0.0
FSCORE_NO_CATALYST_KAVOUT:      float = 0.15

# Momentum Score 세부 가중치 (합 = 1.0)
# 멀티 기간 수익률 추가 — 모멘텀 지속성 반영
FSCORE_MOM_RSI_WEIGHT:    float = 0.20   # RSI(14) 구간 점수
FSCORE_MOM_RVOL_WEIGHT:   float = 0.20   # Relative Volume
FSCORE_MOM_52W_WEIGHT:    float = 0.20   # 52W 고점/저점 위치
FSCORE_MOM_SMA_WEIGHT:    float = 0.20   # SMA20/50/200 위치 (추세)
FSCORE_MOM_RETURN_WEIGHT: float = 0.20   # 멀티 기간 수익률 (3M/6M/12M)
